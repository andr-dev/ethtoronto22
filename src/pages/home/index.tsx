...
import { AppActionType, appContext } from "../../context";
import { ethersProvider } from "../../util/ethers-service";
...
const Home: React.FC = () => {
  ...
  const generateChallenge = (address: any) => {
  ...
  const login = async (): Promise<{
    ...
    const signature = await ethersProvider.getSigner().signMessage(challengeResponse.data.challenge.text);

    const { data } = await authenticate(address, signature);

    return data && (data as any).authenticate
      ? Promise.resolve({
          accessToken: (data as any).authenticate.accessToken,
          refreshToken: (data as any).authenticate.refreshToken,
        })
      : Promise.reject();
  };
  ...
  return (
    <div className="flex h-screen bg-white py-6 sm:py-8 lg:py-12">
      <div className="max-w-screen-2xl px-4 md:px-8 m-auto">
